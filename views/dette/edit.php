<?php
$titre = 'Modifier la dette';
$enTete = 'Modifier la dette';
...
<div class="max-w-xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <form action="<?= WEBROOT ?>/dettes/modifier" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="utilisateur_id"
                    ...
                    <?php foreach ($clients as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= ($dette['utilisateur_id'] == $c['id']) ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <input type="text" name="dette" value="<?= htmlspecialchars($dette['dette']) ?>"
            ...
            <div>
                ...
                <input type="number" step="0.01" name="montant" value="<?= htmlspecialchars($dette['montant']) ?>"
            ...
            <div>
                <label class="block text-xs font-medium text-slate-500 mb-1.5">État</label>
                <select name="etat" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="non_solde" <?= $dette['etat'] === 'non_solde' ? 'selected' : '' ?>>Non soldée</option>
                    <option value="solde" <?= $dette['etat'] === 'solde' ? 'selected' : '' ?>>Soldée</option>
                </select>
            </div>
